...
## 2025-03-02 - Optimize Godot Node Management\n**Learning:** Frequent instantiation and freeing of nodes (like `MeshInstance3D`) during active gameplay loop causes garbage collection stutter and CPU spikes. Specifically, `queue_free()` and `instantiate()` over complex nodes are heavy.\n**Action:** Use Object Pooling for nodes that are repeatedly spawned and destroyed (e.g., terrain chunks). When a node goes out of view, set it invisible and store it in an array instead of freeing it. When a new node is needed, reuse from the array if possible.
