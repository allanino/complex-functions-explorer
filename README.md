...
Defined as $\zeta(s) = \sum_{n=1}^\infty n^{-s}$ for $\mathrm{Re}(s) > 1$, it is analytically continued to the rest of the complex plane. Our implementation uses the **Dirichlet Eta representation** for numerical stability when $\mathrm{Re}(s) > 0.5$:
$$\zeta(s) = \frac{1}{1 - 2^{1-s}} \sum_{n=1}^\infty \frac{(-1)^{n-1}}{n^s}$$

For $\mathrm{Re}(s) \le 0.5$, the explorer utilizes the **reflection formula** to achieve analytical continuation to the entire complex plane:
$$\zeta(s) = 2^s \pi^{s-1} \sin\left(\frac{\pi s}{2}\right) \Gamma(1-s) \zeta(1-s)$$
In this calculation, the term $\zeta(1-s)$ is evaluated using the Dirichlet Eta representation, since for $\mathrm{Re}(s) \le 0.5$, the reflected point $1-s$ has a real part greater than or equal to $0.5$. This allows evaluation across the critical strip and beyond.
