...
func _ready():
	...
	apply_aa()
	menu_overlay._disable_sliders_focus(menu_overlay)
	_last_zeros_visible = Config.show_hud_zeros

	menu_overlay.apply_aa_signal.connect(apply_aa)
	menu_overlay.portal_flash_signal.connect(play_portal_flash)
	menu_overlay.update_hud_layout_signal.connect(_update_hud_layout)
	menu_overlay.toggle_menu_signal.connect(toggle_menu)
