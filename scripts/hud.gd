...
@onready var custom_palette_input = $Control/MenuOverlay/CenterContainer/MainPanel/MarginContainer/ContentVBox/TabContainer/ENVIRONMENT/CustomPaletteContainer/CustomPaletteInput
@onready var browse_button = $Control/MenuOverlay/CenterContainer/MainPanel/MarginContainer/ContentVBox/TabContainer/ENVIRONMENT/CustomPaletteContainer/BrowseButton
@onready var palette_mapping_button = $Control/MenuOverlay/CenterContainer/MainPanel/MarginContainer/ContentVBox/TabContainer/ENVIRONMENT/PaletteMappingContainer/PaletteMappingButton
@onready var palette_scale_input = $Control/MenuOverlay/CenterContainer/MainPanel/MarginContainer/ContentVBox/TabContainer/ENVIRONMENT/PaletteScaleContainer/PaletteScaleInput
@onready var palette_file_dialog = $TooltipLayer/PaletteFileDialog
...
const DESCRIPTIONS = {
	...
	"Custom Palette": "Path to a 2D image file to be used as the domain coloring texture.",
	"Mapping Mode": "Choose between 1D Cyclic (phase-based) and 2D Complex (value-based) mapping.",
	"Map Scale": "Scale factor for 2D complex mapping.",
...
func _ready():
	...
	palette_mapping_button.clear()
	palette_mapping_button.add_item("Cyclic (Phase)")
	palette_mapping_button.add_item("2D Complex (Value)")

	browse_button.pressed.connect(_on_browse_palette_pressed)
...
func _any_dropdown_popup():
	return (
		...
		|| palette_mapping_button.get_popup().visible
		|| environment_button.get_popup().visible
	)

func _on_browse_palette_pressed():
	palette_file_dialog.popup_centered()

func _on_palette_file_selected(path: String):
	custom_palette_input.text = path
...
func toggle_menu(applied: bool = false):
	...
	if menu_overlay.visible:
		...
		custom_palette_input.text = Config.custom_palette_path
		palette_mapping_button.selected = Config.palette_mapping_mode
		palette_scale_input.text = "%.3f" % Config.palette_2d_scale
...
func _on_set_pos_pressed():
	...
	Config.custom_palette_path = custom_palette_input.text
	Config.palette_mapping_mode = palette_mapping_button.selected
	var p_scale = float(palette_scale_input.text)
	if is_finite(p_scale): Config.palette_2d_scale = p_scale
...
func _process(_delta):
	...
	material.set_shader_parameter("palette_mapping_mode", Config.palette_mapping_mode)
	material.set_shader_parameter("palette_2d_scale", Config.palette_2d_scale)
